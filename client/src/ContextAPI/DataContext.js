...
const DataState = (props) => {
  ...
  //     //   .firestore()
  //     //   .collection("AutoSEO_Blogs")
  //     //   .doc(domain);
  //     const docRef = doc(db, "AutoSEO_Blogs", domain);
  //     // const doc = await docRef.get();

  //     if (docRef.exists) {
  //       // setBlogs(doc.data().blogs); // Assuming 'blogs' is the field name containing the blogs
  //       // setBlogsFunction({ data: docRef.data().blogs });
  //       console.log("ContextAPI: ", blogs);
  //     } else {
  //       console.log("No blogs found for this domain");
  //     }
  //   } catch (error) {
  ...
  const fetchBlogs = async () => {
    try {
      ...
      if (docSnap.exists()) {
        // Assuming 'blogs' is the field name containing the blogs
        // businessMetaData;
        setBlogsFunction({ data: docSnap.data().blogs });
        setBusinessMetaDataFunction({ data: docSnap.data().businessMetaData });
        console.log("Blogs fetched for domain:", domain);
        console.log("Blogs fetched :", docSnap.data());
        return {
          blogs: docSnap.data().blogs,
          businessMetaData: docSnap.data().businessMetaData,
        };
      } else {
        console.log("No blogs found for this domain");
        return "";
  ...
  //   fetchBlogs();
  // }
  // async function checkAndCreateDomainDocument(domain, blog) {
  //   const docRef = doc(db, "AutoSEO_Blogs", domain);

  //   // const docRef = db.collection("AutoSEO_Blogs").doc(domain);

  //   try {
  //     // const doc = await docRef.get();

  //     console.log(
  //       `Document for ${domain} does not exist. Creating a new document.`
  //     );
  //     // await docRef.set(blog);
  //     await setDoc(docRef, { blog }, { merge: true });
  //     console.log(`Document created for ${domain}`);
  //   } catch (error) {
  //     console.error("Error accessing Firestore:", error);
  //   }
  // }
  // checkAndCreateDomainDocument("localhost", blogs);
  return (
